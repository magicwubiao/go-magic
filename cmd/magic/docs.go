...
func generateCLIUsageGuide() error {
	...
	buf.WriteString("|----------|-------------|\n")
	buf.WriteString("| `GO_MAGIC_HOME` | Custom magic home directory (default: `~/.magic`) |\n")
	buf.WriteString("| `GO_MAGIC_CORS_ORIGINS` | Extra CORS origins for the dashboard (comma separated) |\n")
	buf.WriteString("| `MAGIC_SKILL_DIR` | Override the skills directory |\n")
	buf.WriteString("| `MAGIC_SESSION_ID` | Pin the session id to use |\n")
	buf.WriteString("\n")
	buf.WriteString("Provider keys are read from their conventional variables as well:\n")
	buf.WriteString("`OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `DEEPSEEK_API_KEY`, `GOOGLE_API_KEY`,\n")
	buf.WriteString("`GROQ_API_KEY`, `OPENROUTER_API_KEY`, `TOGETHER_API_KEY`.\n\n")
	buf.WriteString("Note: profile, config path, verbose/debug and no-color are **flags**\n")
	buf.WriteString("(`--profile/-p`, `--config`, `--verbose/-v`, `--debug`, `--no-color`), not env vars.\n")
	buf.WriteString("\n")
