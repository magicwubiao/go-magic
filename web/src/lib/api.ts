...
// (e.g. https://mission-control.tilos.com/magic/). The Python backend
// injects ``window.__MAGIC_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__MAGIC_BASE_PATH__ ?? "";
  if (!raw) return "";
...
declare global {
  interface Window {
    __MAGIC_SESSION_TOKEN__?: string;
    __MAGIC_BASE_PATH__?: string;
  }
}
let _sessionToken: string | null = null;
const SESSION_HEADER = "X-Magic-Session-Token";
...
export async function fetchJSON<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  const token = window.__MAGIC_SESSION_TOKEN__;
  if (token) {
...
async function getSessionToken(): Promise<string> {
  ...
  const injected = window.__MAGIC_SESSION_TOKEN__;
  if (injected) {
    _sessionToken = injected;
    return _sessionToken;
  }
  throw new Error("Session token not available — page must be served by the Magic dashboard server");
}
...
export const api = {
  ...
  updateMagic: () =>
    fetchJSON<ActionResponse>("/api/magic/update", { method: "POST" }),
