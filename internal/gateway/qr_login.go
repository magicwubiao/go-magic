...
func (m *QRCodeManager) pollPlatformStatus(ctx context.Context, platform string) (string, error) {
	...
	switch platform {
	case "wechat_ilink":
		return m.pollILinkStatus(ctx, session.QRData)
	case "whatsapp":
		return m.pollWhatsAppStatus(ctx)
	case "wechat":
		return m.pollWeChatStatus(ctx, session.QRData)
	default:
		// For unsupported platforms, just return current status
		return session.Status, nil
	}
}

// pollILinkStatus polls iLink API for QR status
func (m *QRCodeManager) pollILinkStatus(ctx context.Context, qrcode string) (string, error) {
...
func (m *QRCodeManager) pollWhatsAppStatus(ctx context.Context) (string, error) {
	// WhatsApp status is tracked via the WhatsAppGateway
	// When WhatsAppGateway is running, it will update the session status
...
func (m *QRCodeManager) pollWeChatStatus(ctx context.Context, qrcode string) (string, error) {
	// WeChat official account status polling
	return "", nil
}
