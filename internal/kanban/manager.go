...
func (m *Manager) DecomposeGoal(ctx context.Context, goalID, goalTitle, goalDescription string, prov provider.Provider) ([]*Task, error) {
	...
	prompt := fmt.Sprintf(`You are an expert project planner. Break down the following goal into a list of specific, actionable tasks that can be tracked on a kanban board.

Goal: %s
Description: %s

Please decompose this goal into 4-8 concrete tasks. Each task should:
1. Have a clear, specific title (verb + object)
2. Include a detailed description explaining what needs to be done
3. Have an estimated number of hours to complete
4. Have a priority level (0=low, 1=medium, 2=high, 3=critical)
5. Be ordered logically (tasks that must be done first should appear first)
...
Only respond with the JSON, no additional text.`, goalTitle, goalDescription)
	...
	if len(result.Tasks) == 0 {
		return nil, fmt.Errorf("no tasks generated by LLM")
	}
	...
	for _, t := range result.Tasks {
		...
		if err != nil {
			log.Warnf("[Kanban] Failed to create task for goal: %v", err)
			continue
		}

		// Link to goal and set estimated hours
		updates := map[string]interface{}{
			"goal_id":         goalID,
			"estimated_hours": t.EstimatedHours,
		}
		updatedTask, err := m.UpdateTask(task.ID, updates)
		if err == nil {
			task = updatedTask
		}
	...
	log.Infof("[Kanban] Goal %s decomposed into %d tasks", goalID, len(tasks))
	return tasks, nil
}
...
// extractJSON extracts the outermost JSON object or array from a string,
// handling markdown code blocks (```json ... ```, ``` ... ```) and surrounding text.
func extractJSON(s string) string {
	...
	return strings.TrimSpace(s[startIdx:endIdx])
}
